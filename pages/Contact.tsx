        ...
        <div className="max-w-6xl mx-auto">
          <header className="mb-24 md:mb-32">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
              <span className="font-inter text-[10px] md:text-xs tracking-[0.6em] text-zinc-500 uppercase mb-4 block">AVAILABLE WORLDWIDE</span>
              <h1 className="font-cinzel text-4xl md:text-7xl lg:text-8xl tracking-tight mb-8">
                LET'S <span className="italic">BEGIN</span> <br />
                THE <span className="opacity-60">STORY</span>.
              </h1>
            </motion.div>
          </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
            <div className="lg:col-span-5 space-y-16">
              <section>
                <h2 className="font-cinzel text-xs tracking-[0.4em] text-zinc-600 uppercase mb-10">DIRECT INQUIRIES</h2>
                <address className="not-italic space-y-10">
                  <a href="mailto:info@dynamicyomifilms.com" className="group block">
                    <p className="text-[9px] tracking-[0.2em] text-zinc-500 uppercase mb-2">Electronic Mail</p>
                    <p className="font-cinzel text-lg md:text-2xl tracking-wider group-hover:text-zinc-400 transition-colors">info@dynamicyomifilms.com</p>
                  ...
                  <a href="tel:+18183008848" className="group block">
                    <p className="text-[9px] tracking-[0.2em] text-zinc-500 uppercase mb-2">Voice Correspondence</p>
                    <p className="font-cinzel text-lg md:text-2xl tracking-wider group-hover:text-zinc-400 transition-colors">+1 818 300- 8848</p>
                    <div className="h-[1px] w-0 group-hover:w-full bg-white transition-all duration-500 mt-2" />
                  </a>

                  <div className="group block">
                    <p className="text-[9px] tracking-[0.2em] text-zinc-500 uppercase mb-2">Creative Hub</p>
                    <p className="font-cinzel text-lg md:text-2xl tracking-wider uppercase">Dallas, Texas</p>
                  </div>
                </address>
              </section>

              <div className="p-8 border border-zinc-900 bg-zinc-950/30 rounded-sm">
                <h3 className="font-cinzel text-[10px] tracking-[0.3em] text-white mb-4 flex items-center gap-2">
                  <MapPin size={12} /> STUDIO HOURS
                </h3>
                <p className="font-inter text-xs text-zinc-500 tracking-wider">
            ...
            <section className="lg:col-span-7">
              <h2 className="font-cinzel text-xs tracking-[0.4em] text-zinc-600 uppercase mb-10">THE ECOSYSTEM</h2>
              <nav className="grid grid-cols-2 gap-4 md:gap-6" aria-label="Social media links">
                {socials.map((social, idx) => (
                  <motion.a key={idx} href={social.link} target="_blank" rel="noopener noreferrer" className="group relative p-4 md:p-8 border border-zinc-900 hover:border-zinc-700 bg-zinc-950/20 transition-all duration-500 flex flex-col justify-between aspect-square h-auto md:h-64">
                    <div className="flex justify-between items-start">
                      <div className="p-2 md:p-3 rounded-lg md:rounded-xl" style={{ color: social.color, backgroundColor: `${social.color}10` }}>
                        {social.icon}
                      </div>
                      <ArrowUpRight size={14} className="text-zinc-800 group-hover:text-white" />
                    </div>
                    <div>
                      <h3 className="font-cinzel text-[10px] md:text-sm tracking-[0.2em] text-white mb-1">{social.name}</h3>
                      <p className="font-inter text-[8px] md:text-[10px] text-zinc-500 tracking-widest">{social.handle}</p>
                    </div>
